 // monthpicker.js
         'use strict'; var Monthpicker = function () {
             function c(a, b) {
                 ...
                 this.id = c.next_id++; c.instances[this.id] = this; this.original_input = a; this.InitOptions(b);
                 this.InitValue(); this.Init(); this.RefreshInputs()
             }
            
             c.Get = function (a) { if ("undefined" === typeof a.parentElement.dataset.mp) throw "Unable to retrieve the Monthpicker of element " + a; return c.instances[a.parentElement.dataset.mp] }; c.prototype.InitValue = function () {

               
                 var a = new Date; this.currentYear = 2019;
                    // a.getFullYear();
                 var b = !1; this.original_input.value.match("[0-9]{1,2}/[0-9]{4}") && (b = this.original_input.value.split("/"),
                 this.selectedMonth = parseInt(b[0]), this.currentYear = this.selectedYear = parseInt(b[1]), b = !0); this.opts.allowNull || b || (this.selectedMonth = a.getMonth(), this.selectedYear = 2019, null !== this.bounds.min.year && (this.selectedYear < this.bounds.min.year ? (this.selectedYear = this.bounds.min.year, this.selectedMonth = this.bounds.min.month ? this.bounds.min.month : 1) : this.selectedYear == this.bounds.min.year &&
             ...
             c.prototype.InitOptions = function (a) {
             ...
             }; c.prototype.UpdateOptions = function (a) { this.MergeOptions(a); this.EvaluateOptions(); this.RefreshUI() }; c.prototype.MergeOptions = function (a) { if (a) for (var b in a) this.opts[b] = a[b] }; c.prototype.EvaluateOptions = function () {
                 var a = { min: { year: null, month: null }, max: { year: null, month: null } }; if (null !== this.opts.minValue || null !== this.opts.minYear) if (null !== this.opts.minValue && null !== this.opts.minYear) {
                     var b = this.opts.minValue.split("/"), c = parseInt(this.opts.minYear), d = parseInt(b[1]); c > d ? (a.min.year =
                 ...
                 this.opts.maxValue ? (b = this.opts.maxValue.split("/"), a.max.year = parseInt(b[1]), a.max.month = parseInt(b[0])) : (a.max.year = parseInt(this.opts.maxYear), a.max.month = 12); this.bounds = a
             ...
             }; c.prototype.RefreshUI = function () { this.UpdateCalendarView(); null !== this.currentYear && (this.year_input.innerHTML = this.currentYear.toString()); this.UpdateYearSwitches() }; c.prototype.InitIU = function () {
                 this.parent = document.createElement("div"); this.parent.classList.add("monthpicker"); this.parent.tabIndex = -1; var a = getComputedStyle(this.original_input, null); this.parent.style.width = a.getPropertyValue("width"); "auto" === this.parent.style.width && (this.parent.style.width =
                 ...
                 "px"); this.input.style.padding = a.getPropertyValue("padding"); this.input.style.border = a.getPropertyValue("border"); this.parent.appendChild(this.input); this.selector = document.createElement("div"); this.selector.classList.add("monthpicker_selector"); this.selector.style.display = "none";
                 for (var a = "<table><tr><td><span class='yearSwitch down'>&lt;</span></td><td><div class='yearValue'>" + this.currentYear + "</div> </td><td><span class='yearSwitch up'>&gt;</span> </td></tr> ", b = 0; 4 > b; b++)
                     var c = 3 * b, d = this.opts.monthLabels.slice(c, c + 3), a = a + ("<tr><td class='month month" + (c + 1) + "' data-m='" + (c + 1) + "'>" + d[0] + "</td><td class='month month" + (c + 2) + "' data-m='" + (c + 2) + "'>" + d[1] + "</td><td class='month month" + (c + 3) + "' data-m='" + (c + 3) + "'>" + d[2] + "</td></tr>"); this.selector.innerHTML = a + "</table>"; this.parent.appendChild(this.selector)
             };


             c.prototype.Init = function () {
                 this.InitIU(); this.year_input = this.selector.querySelector(".yearValue"); this.parent.dataset.mp = this.id.toString(); this.parent.addEventListener("focusin", function () { c.instances[this.dataset.mp].Show() },
                 ...
                 function () { this.classList.contains("off") || c.instances[this.closest(".monthpicker").dataset.mp].SelectMonth(this.dataset.m) })
             ...
             function () {
                 for (var a = this.selector.querySelectorAll(".month"), b = 0; b < a.length; b++) a[b].classList.remove("selected"); null !== this.selectedYear && this.currentYear === this.selectedYear && a[this.selectedMonth - 1].classList.add("selected"); for (b = 0; b < a.length; b++) a[b].classList.remove("off"); if (null !== this.bounds.min.year && this.currentYear <= this.bounds.min.year) for (b = 1; b < this.bounds.min.month; b++) a[b - 1].classList.add("off"); if (null !== this.bounds.max.year && this.currentYear >= this.bounds.max.year) for (b = 12; b > this.bounds.max.month; b--) a[b -
                 1].classList.add("off")

             }; c.prototype.ReleaseFocus = function () { this.parent.blur() }; c.prototype.Show = function ()
             { this.RefreshUI(); this.selector.style.display = "block" }; c.prototype.Hide = function () {
                 null !== this.selectedYear && (this.currentYear = this.selectedYear);
                 this.selector.style.display = "none"
             };
             c.prototype.ShowYear = function (a) { this.currentYear = a; this.RefreshUI() }; c.prototype.UpdateYearSwitches = function () {
            
                 var a = this.selector.querySelector(".yearSwitch.down"), b = this.selector.querySelector(".yearSwitch.up");
              

                 null !==2019 && this.currentYear <=2019 ? a.classList.add("off") : a.classList.remove("off");

                 null !== this.bounds.max.year && this.currentYear >= this.bounds.max.year ? b.classList.add("off") : b.classList.remove("off")
             };

             c.prototype.PrevYear = function () { this.ShowYear(this.currentYear - 1) };
             c.prototype.NextYear = function () { this.ShowYear(this.currentYear + 1) }; c._clone = function (a) {
                 var b; if (null == a || "object" != typeof a) return a; if (a instanceof Date) return b = new Date, b.setTime(a.getTime()), b; if (a instanceof
                 Array) { b = []; for (var e = 0, d = a.length; e < d; e++) b[e] = c._clone(a[e]); return b } if (a instanceof Object) { b = {}; for (e in a) a.hasOwnProperty(e) && (b[e] = c._clone(a[e])); return b } throw Error("Unable to copy obj! Its type isn't supported.");
         ...
         window.jQuery && (window.jQuery.fn.Monthpicker = function (c, a) {
             var b; if ("undefined" === typeof c || "object" === typeof c) b = "ctor"; else if ("string" === typeof c && "option" === c) b = "option"; else { console.error("Error : Monthpicker - bad argument (1)"); return } window.jQuery(this).each(function (e, d) {
                 switch (b) {
                     case "ctor": "INPUT" != d.tagName || "text" != d.getAttribute("type") && null !== d.getAttribute("type") ? console.error("Monthpicker must be called on a text input") : new Monthpicker(d, c); break; case "option": "INPUT" != d.tagName ||
